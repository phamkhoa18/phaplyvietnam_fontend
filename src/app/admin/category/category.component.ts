  ...
  remove(item: any ) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        (await this.api.post('/del_category', _id)).subscribe((v : any ) => {
            if(v.status == 200) {
              this.data.notification('Xóa danh mục thành công ' , 'Xóa danh mục thành công' , 'success')
              this.getData();
            } else {
              this.data.notification('Xóa danh mục không thành công' , 'Xóa danh mục không thành công' , 'error')
              this.getData();
            }
        });
      }
